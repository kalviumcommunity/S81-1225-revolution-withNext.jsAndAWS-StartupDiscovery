  ...
  try {
    ...
    if (users.length === 0) {
      ...
      users.forEach((user, index) => {
        ...
        console.log(`      Startups: ${user._count.startups}, Comments: ${user._count.comments}`);
      });
    }
    console.log('');
    ...
    if (startups.length === 0) {
      ...
      startups.forEach((startup, index) => {
        ...
        const categories = startup.categories.map(sc => sc.category.name).join(', ');
        console.log(`      Categories: ${categories || 'None'}`);
        
        const tags = startup.tags.map(st => st.tag.name).join(', ');
        console.log(`      Tags: ${tags || 'None'}`);
        
        console.log(`      Engagement: ${startup._count.votes} votes, ${startup._count.comments} comments, ${startup._count.bookmarks} bookmarks`);
    ...
    console.log('   ├─ Avg Votes:         ', stats._avg.voteCount?.toFixed(1) || 0);
    console.log('   ├─ Avg Views:         ', stats._avg.viewCount?.toFixed(1) || 0);
    ...
    if (recentComments.length === 0) {
      ...
      recentComments.forEach((comment, index) => {
        ...
        console.log(`   ${prefix} ${comment.user.username} on "${comment.startup.title}"`);
        console.log(`      "${preview}"`);
      });
    }
    console.log('');
